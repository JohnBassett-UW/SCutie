#' importX
#' import count matrices generated by Cell Ranger or CITEseq count.
#' 	some other count matrix formats are supported depending on
...
#'
#' @return Sparse matrix of single-cell count data. Class dgCMatrix.
#' @export
#'
#' @examples
#' path_to_file <- '/path/to/data'
#' data <- importX(path_to_file)
#'
#' path_to_file <- '/path/to/data.h5'
#' data <- importX(path = path_to_file, ESNG = T)
importX <- function(path, type = "unspecified", ESNG = F, garbage_collection = T){
  ...
  dimnames_unique <- function(ct_data){
    if(any(duplicated(dimnames(ct_data)[[2]]))){
      warning("duplicate UMI's found in data set")
    }

    dupes <- duplicated(dimnames(ct_data)[[1]]) #logical vector of duplicate indices

    if(any(dupes)){
      message("Warning in column names: gene symbols contain duplicate identities")
      dup_names <- dimnames(ct_data)[[1]][dupes] #character vector of duplicate names
      dimnames(ct_data)[[1]][dupes] <- generate_unique_names(dup_names)
  ...
  return(dimnames_unique(ct_data))
}
